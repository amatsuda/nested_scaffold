...
class <%= controller_class_name %>ControllerTest < ActionController::TestCase
  ...
  test "should show <%= singular_table_name %>" do
    get :show, :<%= nested_parent_id %> => @<%= nested_parent_name %>, :id => @<%= singular_table_name %>
    assert_response :success
  end

  test "should get edit" do
    get :edit, :<%= nested_parent_id %> => @<%= nested_parent_name %>, :id => @<%= singular_table_name %>
    assert_response :success
  end

  test "should update <%= singular_table_name %>" do
    put :update, :<%= nested_parent_id %> => @<%= nested_parent_name %>, :id => @<%= singular_table_name %>, :<%= singular_table_name %> => @<%= singular_table_name %>.attributes
    assert_redirected_to <%= nested_parent_name %>_<%= singular_table_name %>_path(@<%= nested_parent_name %>, assigns(:<%= singular_table_name %>))
  end

  test "should destroy <%= singular_table_name %>" do
    assert_difference('<%= class_name %>.count', -1) do
      delete :destroy, :<%= nested_parent_id %> => @<%= nested_parent_name %>, :id => @<%= singular_table_name %>
    end
